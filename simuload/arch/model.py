...
class Model:
    ...
    def inserir_transformador(self, transformador):
        """Adiciona uma nova linha na tabela.
        ...
        try:
            self.database.cur.execute(sql, transformador)
        except Exception as e:
            print("\n[x] Falha ao inserir registro [x]\n")
            print(f"[x] Revertendo operação (rollback) [x]: {e}\n")
            # rollback reverte/desfaz a operação.
    ...
    def modificar_transformador(self, rowid, transformador):
        """Alterar uma linha da tabela com base na id.
        A query está configurada para alterar apenas o nome e sexo.
        :param rowid (int): id da linha que se deseja alterar.
        :param transformador (tuple): Tupla contendo os dados.
        ...
        try:
            self.database.cur.execute(
                """UPDATE Transformador SET TransformadorNome=?, TransformadorDemanda=?, TransformadorFornecimento=?
        WHERE rowid=?""",
                (*transformador, rowid),
            )
        except Exception as e:
    ...
    def consultar_tranformador_pela_id(self, rowid):
        """Consulta registro pela id.
        :param rowid (int): id do usuário que se deseja consultar.
        :return: É retornada uma tupla (tuple) com os dados.
        Caso o registro não seja localizado é retornado ``None``.
    ...
    def remover_transformador(self, rowid):
        ...
        """
        try:
            self.database.cur.execute(f"""DELETE FROM Transformador WHERE rowid=?""", (rowid,))
        except Exception as e:
